...
def extract_context_entities(script: str) -> dict:
    ...
    if not country_context:
        ...
        if not country_context:
            ...
            # Strip possessive apostrophe-s before matching
            # "Donald Trump's" → "donald trump"
            # "Biden's" → "biden"
            cleaned_persons = [
                p.lower()
                 .replace("'s", "")
                 .replace("\u2019s", "")   # curly apostrophe
                 .strip()
                for p in persons
            ]
    ...
    # Normalize possessives in the persons list
    persons = [
        p.replace("'s", "").replace("\u2019s", "").strip()
        for p in persons
    ]
    # Remove any persons that became empty after strip
...
def merge_short_scenes(scenes):
    """
    Merge scenes shorter than MIN_WORDS into adjacent scenes.
    Guards: merged result must never exceed MAX_WORDS.
    Short scenes that would cause overflow are kept as-is.
    """
    merged = []
    buffer = ""
    buffer_meta = None
        ...
        if words < MIN_WORDS:
            ...
            if buffer:
                ...
                if len(candidate.split()) <= MAX_WORDS:
                    ...
                    # Merge would overflow — keep buffer as own scene
                    buf_text = buffer.strip()
                    if len(buf_text.split()) >= 3:
                        merged.append({
                            **(buffer_meta or scene),
                            "text": buf_text
                        })
                    buffer = ""
                    buffer_meta = None
    ...
    if buffer.strip():
        ...
        if buf_words and len(buf_words) >= 3:
            if merged and len(merged[-1]["text"].split()) + len(buf_words) <= MAX_WORDS:
                ...
                merged.append({
                    **(buffer_meta or (merged[-1] if merged else {})),
                    "text": buffer.strip()
                })
...
def _clean_scene_text(text: str) -> str:
    ...
    # Strip leading "To" when followed by a quote or lowercase word.
    # e.g. 'To "severe" for...' → '"Severe" for...'
    # e.g. 'To continue...' → 'Continue...'
